...
def train_and_predict(train_file, test_file, output_file):
    try:
        ...
        # Ensure test data contains all required columns
        missing_cols = set(X.columns) - set(test_data.columns)
        for col in missing_cols:
            test_data[col] = 0  # Fill missing columns with zeros
...
def process():
    try:
        ...
        crate_path = r"grossweight\crate.csv"
        jan_path = r"outliers_free_jan_data_transformed.csv"
        output1 = "output.csv"
        output2 = "output2.csv"
        result = "result.csv"
